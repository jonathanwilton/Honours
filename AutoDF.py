...
class oracle(object):
    ...
    def __init__(self, dist = ['Normal','Laplace', 't5', 'Cauchy'], device = 
                 'cpu', m = 500, num = 30, epochs = 300, N = 30, L = [0,1,2], 
                 D = [10,50], lag = [1,2,5,10,20,50,100,200], tol = [0.5,0.05,5,20], PI = 
        ...
        device : 'cpu', 'cuda', default = 'cpu'
            Device to train the models on. There are currently bugs with GPU 
            support so CPU is recommended.
        ...
        PI : iterable of proportions, default = [0.5, 0.67, 0.95, 0.99]
            The coverage probabiolities for prediction intervals that are used 
            in Bayesian Optimisation if 'uncertainty' is included in the metric 
            in the .fit method. The pinball loss with the validation data is 
            used to assess the fit of the prediciton intervals. The 50% and 67% 
            prediction intervals are useful for assessing if the middle of the 
            conditional distribution is learnt well. The 95% and 99% prediction 
            intervals are useful for assessing if the tails of the conditional 
            distribution are learnt well.
            ...
            def sampler(context, m):
                ...
                if not (np.isnan(torch.min(p).item()) or np.isinf(torch.min(p).item())):
                    if ((torch.min(p).item() >= 0) and (torch.max(p).item() <= 1)):
                        ...
                        # print('min p:',torch.min(p).item(),', max p:',torch.max(p).item())
...
#%%
    def plot(self, point, intervals, test = None, plot_ind = False, ind_points = None, legend = True, save_fig = False):
